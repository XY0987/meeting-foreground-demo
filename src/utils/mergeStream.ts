...
export class VideoStreamMerger {
  ...
  constructor(options: Partial<ConstructorOptions> = {}) {
    ...
    const audioSupport = !!(
      window.AudioContext && new AudioContext().createMediaStreamDestination
    );
    ...
    const audioDestination = (this._audioDestination =
  ...
  private _createConstantSource() {
    if (this._audioCtx) {
      ...
      const constantBuffer = this._audioCtx.createBuffer(
        1,
        1,
   ...
   */
  updateIndex(
    mediaStream: MediaStream | string | { id: string },
    index: number,
  ): void {
  ...
  addMediaElement(id: string, element: HTMLMediaElement, opts: any): void {
    ...
    if (this._audioCtx && !opts.mute) {
      const audioSource =
        element._mediaElementSource ||
        this._audioCtx.createMediaElementSource(element);
  ...
  addStream(
    ...
    opts: AddStreamOptions | undefined,
  ): void {
    if (typeof mediaStream === "string") {
    ...
    if (!videoElement) {
      ...
      if (stream.hasAudio && this._audioCtx && !stream.mute) {
        stream.audioSource =
          this._audioCtx.createMediaStreamSource(mediaStream);
